//
//  multiply - multiply of two images
//  
	...
	switch(f1.pixelformat)
	{
		case 16:
			...
			for(y = 0; y < h; y++) 
			{
				...
				for(x = 0; x < w; x++)
				{
					pix2_16[x] = rgbtocolor16(klamp255(r16(pix1_16[x])*r16(pix2_16[x])/255), 
						                      klamp255(g16(pix1_16[x])*g16(pix2_16[x])/255), 
											  klamp255(b16(pix1_16[x])*b16(pix2_16[x])/255));
				}
			}
			break;
		case 24:
			for(y = 0; y < h; y++) 
			{
				...
				for(x = 0; x < w; x++)
				{
					pix2_24[x] = rgbtocolor24(klamp255(r24(pix1_24[x])*r24(pix2_24[x])/255), 
						                      klamp255(g24(pix1_24[x])*g24(pix2_24[x])/255), 
											  klamp255(b24(pix1_24[x])*b24(pix2_24[x])/255));
				}
			}
			break;
		case 32:
			for(y = 0; y < h; y++) 
			{
				...
				for(x = 0; x < w; x++)
				{
					pix2_32[x] = rgbtocolor32(klamp255(r32(pix1_32[x])*r32(pix2_32[x])/255), 
						                      klamp255(g32(pix1_32[x])*g32(pix2_32[x])/255), 
											  klamp255(b32(pix1_32[x])*b32(pix2_32[x])/255));
				}
			}
			break;
	}
