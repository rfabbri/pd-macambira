...
static int _pdp_packet_convert_image_to_matrix(int packet, t_pdp_symbol *dest_template, int type)
{
	...
	case PDP_MATRIX_TYPE_CDOUBLE:
	    ...
	    break;
	default:
	    post("_pdp_packet_convert_image_to_matrix: INTERNAL ERROR");
	}
	break;
    default:
	pdp_packet_mark_unused(new_p);
...
{
    post("can't convert image type %s to %s",
...
void pdp_matrix_setup(void)
{
    ...
    matrix_class->clone = _pdp_matrix_clone;
    matrix_class->reinit = _pdp_matrix_reinit;
	...
	post("pdp_matrix_setup: WARNING: overriding gsl error handler.");
    }    
