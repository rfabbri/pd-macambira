...
static int _open_read(t_png_image* x, char *file)
{
    ...
    x->x_fp = fopen(file, "r");
    if (!x->x_fp) {
	D post("can't open %s for reading", file);
	return 0;
    }
    fread(header, 1, SIG_BYTES, x->x_fp);
    is_png = !png_sig_cmp(header, 0, SIG_BYTES);

    D post("%s is %s png file", file, is_png ? "a" : "not a");
...
static int _open_write(t_png_image* x, char *file)
{
    ...
    x->x_fp = fopen(file, "w");
    if (!x->x_fp) {
	D post("can't open %s for writing", file);
	return 0;
    }
...
static int _checkimagetype(t_png_image *x)
{
    ...
    D post("image info: w=%d, h=%d, depth=%d, type=%d",
    ...
    if (x->x_bit_depth < 8) {
	png_set_expand(x->x_png);
	D post("convert greyscale to 8 bit");
    }
    if (x->x_bit_depth == 16){
	D post("stripping 16 bit to 8 bit");
	...
	if (png_get_valid(x->x_png, x->x_info, PNG_INFO_tRNS)){
	    D post("no support for greyscale images with alpha info");
	    return 0;
	}
	return 1;
    }
    ...
    D post("image type not supported");
    return 0;
...
static int _getimagedata(t_png_image *x)
{
    ...
    D post("reading %d rows ", (int)x->x_height);
...
static int _saveimagedata(t_png_image *x, int packet)
{
    ...
    D post("writing %d rows ", (int)x->x_height);
...
int pdp_packet_bitmap_save_png_file(int packet, char *filename)
{
    post("WARNING: no png support, can't save png file %s", filename);
    return 0;
}

int pdp_packet_bitmap_from_png_file(char *filename)
{
    post("WARNING: no png support, can't load png file %s", filename);
    return -1;
}
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) 
	post("usage: %s <png file>", argv[0]);
    else
	image = load_png(argv[1]);

    post ("%s", image ? "OK" : "ERROR");
