...
Fifo::Fifo(int s, gavl_audio_format_t * f) {
	size = s  ;  
	start = 0 ;
	end = 0 ; 
	count = 0;
...
void Fifo::Flush() {
	...
	count = 0;
	//printf("Fifo::flushed size=%d\n", count);
	pthread_mutex_unlock (&mut);
}

// push an element onto the Fifo 
bool Fifo::Append(  gavl_audio_frame_t * source) {
	bool ret = false;
...
bool Fifo::Get( gavl_audio_frame_t * dest) {
	bool ret = false;
